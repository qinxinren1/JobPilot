...
FABRICATION_WATCHLIST: set[str] = {
    ...
    "spring", "rails", "angular", "vue", "svelte",
    # Hard lies: certifications can't be stretched
    "certif", "certified", "pmp", "scrum master", "aws certified",
}

REQUIRED_SECTIONS: set[str] = {"TECHNICAL SKILLS", "EXPERIENCE", "PROJECTS", "EDUCATION"}  # SUMMARY removed
...
def validate_json_fields(data: dict, profile: dict, mode: str = "normal") -> dict:
    ...
    # Note: education is not required as it's populated from profile
    # Note: summary is no longer required (removed from tailored resumes)
    for key in ("title", "skills", "experience", "projects"):
        if key not in data or not data[key]:
            errors.append(f"Missing required field: {key}")
    if errors:
        return {"passed": False, "errors": errors, "warnings": warnings}

    # Collect all text for bulk checks (no summary)
    all_text_parts: list[str] = []

    # Skills: check for fabrication and minimum count (always enforced)
    if isinstance(data["skills"], dict):
        ...
        # Check minimum skills count (at least 15 total skills across all categories)
        total_skills = 0
        for category_items in data["skills"].values():
            if isinstance(category_items, list):
                total_skills += len(category_items)
            elif isinstance(category_items, str):
                # Handle string format (comma-separated)
                total_skills += len([s.strip() for s in category_items.split(",") if s.strip()])
        
        if total_skills < 15:
            errors.append(f"Skills section too sparse: only {total_skills} skills found. Minimum required: 15 skills across all categories.")
    ...
    if isinstance(data["experience"], list):
        for company in preserved_companies:
            # Check company field (profile format) or header field (legacy format)
            has_company = any(
                company.lower() in str(e.get("company", "")).lower() or
    ...
    # Awards: should not be in LLM output (automatically populated from profile)
    if "awards" in data and data["awards"]:
        warnings.append("Awards field should not be in LLM output - awards are automatically populated from profile")

    # Education: preserved schools validation
    # Note: Since education is now populated from profile, we validate against profile data
    # instead of LLM response. This check is done in validate_tailored_resume() instead.
    # We skip validation here as the education field may be omitted from LLM response.
...
def validate_tailored_resume(text: str, profile: dict, original_text: str = "") -> dict:
    ...
    # Note: SUMMARY section is no longer required (removed from tailored resumes)
    ...
    # 5. Check awards preserved
    preserved_awards = resume_facts.get("preserved_awards", [])
    for award in preserved_awards:
        if award and award.lower() not in text_lower:
            errors.append(f"Award '{award}' missing -- awards are preserved facts and cannot be removed")

    # 6. Check schools preserved
    preserved_schools = resume_facts.get("preserved_schools", [])
    # Support legacy format (preserved_school as string) for backward compatibility
    if not preserved_schools:
        legacy_school = resume_facts.get("preserved_school", "")
        if legacy_school:
            preserved_schools = [legacy_school]
    
    for school in preserved_schools:
        if school and school.lower() not in text_lower:
            errors.append(f"Education '{school}' missing")

    # 7. Check contact info preserved (warn, don't error -- we can inject)
    ...
    # 8. Scan TECHNICAL SKILLS section for fabricated tools
    ...
    # 9. Scan full document for fabrication watchlist items not in original
    if original_text:
    ...
    # 10. Em dashes (should be auto-fixed by sanitize_text, but safety net)
    if "\u2014" in text or "\u2013" in text:
        errors.append("Contains em dash or en dash.")

    # 11. Banned words (word-boundary matching)
    ...
    # 12. LLM self-talk leak detection
    found_leaks = [p for p in LLM_LEAK_PHRASES if p in text_lower]
    if found_leaks:
        errors.append(f"LLM self-talk: '{found_leaks[0]}'")

    # 13. Duplicate section detection (summary removed)
