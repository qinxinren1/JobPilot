...
def apply(
    ...
    if not _profile_path.exists():
        console.print(
            ...
            "Please configure your profile in the web interface first."
        )
        raise typer.Exit(code=1)
...
def doctor() -> None:
    ...
    if PROFILE_PATH.exists():
        ...
        results.append(("profile.json", fail_mark, "Configure in web interface"))
    ...
    if RESUME_PATH.exists():
        ...
        results.append(("resume.txt", fail_mark, "Upload in web interface"))

    # Search config
    if SEARCH_CONFIG_PATH.exists():
        results.append(("searches.yaml", ok_mark, str(SEARCH_CONFIG_PATH)))
    else:
        results.append(("searches.yaml", warn_mark, "Will use example config — configure in web interface"))
    ...
    if has_gemini:
        ...
        results.append(("LLM API key", fail_mark,
                        "Set GEMINI_API_KEY in project root .env file"))
