...
APP_DIR = Path(os.environ.get("JOBPILOT_DIR", Path.home() / ".jobpilot"))
...
BASE_COVER_LETTERS_DIR = APP_DIR / "base_cover_letters"
...
def ensure_dirs():
    ...
    for d in [APP_DIR, TAILORED_DIR, COVER_LETTER_DIR, LOG_DIR, CHROME_WORKER_DIR, APPLY_WORKER_DIR, BASE_RESUMES_DIR, BASE_COVER_LETTERS_DIR]:
...
def load_profile() -> dict:
    ...
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Please configure in web interface."
        )
...
def load_env():
    ...
    Priority:
    1. Project root .env (where pyproject.toml or src/jobpilot exists)
    2. Current working directory .env (fallback)
    """
    from dotenv import load_dotenv
    
    # Find project root by looking for pyproject.toml or src/jobpilot
    project_root = None
    current = Path(__file__).parent.parent.parent  # Start from src/jobpilot/config.py -> src/jobpilot -> src -> project root
    if (current / "pyproject.toml").exists() or (current / "src" / "jobpilot").exists():
...
def get_tier() -> int:
    ...
    # Check for claude command (PATH and common locations)
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — set GEMINI_API_KEY in .env file")
    if required >= 3:
