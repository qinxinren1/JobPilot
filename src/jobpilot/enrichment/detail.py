...
def extract_from_json_ld(intel: dict) -> dict | None:
    ...
    Returns {"full_description": str, "application_url": str|None, "title": str|None, "site": str|None, "location": str|None} or None."""
...
Find FIVE things in the HTML below:
1. The full job description text (responsibilities, requirements, etc.)
2. The URL of the "Apply" button/link
3. The job title (e.g., "Senior Software Engineer")
4. The company name (e.g., "Google", "Microsoft")
5. The job location (e.g., "San Francisco, CA" or "Remote")
...
- For title: extract the exact job title/position name
- For company: extract the employer/company name
- For location: extract city, state/province, country, or "Remote" if applicable
- If you cannot find a field, set it to null

Return ONLY valid JSON:
{{"full_description": "the complete job description text here", "application_url": "https://..." or null, "title": "Job Title" or null, "site": "Company Name" or null, "location": "Location" or null}}
...
def extract_with_llm(page, url: str) -> dict:
    ...
    if not content:
        return {"full_description": None, "application_url": None, "title": None, "site": None, "location": None}
...
def scrape_detail_page(page, url: str) -> dict:
    ...
    if desc:
        ...
        # Try to extract title and site from page title
        try:
    ...
    result["site"] = llm_result.get("site")
    result["location"] = llm_result.get("location")
    result["tier_used"] = 3
