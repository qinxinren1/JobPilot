...
def _get_or_generate_cover_letter(job: dict, profile: dict, save_files: bool = True) -> tuple[str, str | None, str | None]:
    """Get or generate cover letter for a job.
    
    Simple wrapper that calls the unified cover letter generation function.
    
    Args:
        job: Job dict with title, site, full_description, role_category
    ...
    """
    from jobpilot.scoring.cover_letter import get_or_generate_cover_letter
    
    # Call unified function - handles all logic (base template lookup, LLM customization/generation, file saving)
    return get_or_generate_cover_letter(
        job=job,
        profile=profile,
        max_retries=2,
...
Decision tree:
1. Job posting shows a range (e.g. "$40K-$60K")? -> Answer with the MIDPOINT ($50K).
2. Title says Senior, Staff, Lead, Principal, Architect, or level II/III/IV? -> Minimum $40K {currency}. Use midpoint of posted range if higher.
3. {convert_line}
...
EEO/demographics -> Use the values from APPLICANT PROFILE section:
...
If the form doesn't have the exact option, choose the closest match or "Prefer not to say" / "Decline to self-identify"."""
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    # Handle both HTML and PDF paths
    if resume_file.suffix.lower() == ".html":
        # If path is HTML, try to find corresponding PDF
        pdf_path = resume_file.with_suffix(".pdf")
        if pdf_path.exists():
            src_pdf = pdf_path.resolve()
        else:
            # PDF doesn't exist, try to generate it from HTML
            logger.warning(f"PDF not found for HTML resume, generating from {resume_file}")
            try:
                ...
                with sync_playwright() as p:
                    ...
                    browser.close()
                src_pdf = pdf_path.resolve()
                logger.info(f"Generated PDF from HTML: {src_pdf}")
            except Exception as e:
                logger.error(f"Failed to generate PDF from HTML: {e}")
                # Fallback to base resume if available
                from jobpilot.config import BASE_RESUMES_DIR
                base_pdfs = list(BASE_RESUMES_DIR.glob("*.pdf")) if BASE_RESUMES_DIR.exists() else []
                if base_pdfs:
                    src_pdf = base_pdfs[0].resolve()
                    logger.info(f"Using fallback base resume: {src_pdf}")
                else:
                    raise ValueError(f"Resume PDF not found: {pdf_path} and failed to generate from HTML")
    else:
        # Assume it's already a PDF path
        src_pdf = resume_file.with_suffix(".pdf").resolve()
    ...
    # Directly call _get_or_generate_cover_letter - it handles everything (base template lookup, LLM customization, saving txt/pdf)
    # If cover_letter parameter is provided, use it; otherwise generate/get from database
    if cover_letter:
        cover_letter_text = cover_letter
        # Check for existing PDF if cover letter text was provided
        cl_path = job.get("cover_letter_path")
        cl_pdf_path = None
        if cl_path and Path(cl_path).exists():
            cl_src = Path(cl_path)
            if cl_src.suffix == ".txt":
                cl_pdf_src = cl_src.with_suffix(".pdf")
                if cl_pdf_src.exists():
                    cl_pdf_path = str(cl_pdf_src)
            elif cl_src.suffix == ".pdf":
                cl_pdf_path = str(cl_src)
    else:
        # Generate or get cover letter (includes saving txt and pdf)
        cover_letter_text, _, cl_pdf_path = _get_or_generate_cover_letter(job, profile, save_files=True)
    
    # Copy PDF to upload directory if available
    cl_upload_path = ""
    if cl_pdf_path and Path(cl_pdf_path).exists():
        cl_upload = dest_dir / f"{name_slug}_Cover_Letter.pdf"
        shutil.copy(str(cl_pdf_path), str(cl_upload))
        cl_upload_path = str(cl_upload)
        logger.info("Prepared cover letter PDF for upload: %s", cl_upload_path)
    ...
    # Dry-run: override submit instruction and mission text
    if dry_run:
        submit_instruction = "*** STOP HERE - DRY RUN MODE ***\n\nDO NOT click Submit, Apply, Send, or any final submission button. DO NOT proceed past this step.\n\nTake a final browser_snapshot to review all filled fields. Verify everything looks correct. Then immediately output RESULT:APPLIED with note 'dry run - form filled but not submitted'.\n\nDO NOT click any button that would submit the application. If you see a submit button, that means you're done - just output the result."
        mission_text = "Fill out the application form completely, but DO NOT submit it. This is a DRY RUN - you are only testing the form filling process. STOP before clicking any Submit/Apply button."
        goal_text = "If something unexpected happens and these instructions don't cover it, figure it out yourself. You are autonomous. Navigate pages, read content, try buttons, explore the site. The goal is to fill out the form completely, but you MUST STOP before clicking Submit/Apply. When you reach the final review page or see a submit button, that's your stopping point - take a snapshot and output the result."
        after_submit_text = "11. *** DRY RUN - DO NOT SUBMIT ***\n\nYou have reached the final step. DO NOT click Submit/Apply/Send or any submission button.\n\nTake a final browser_snapshot to review all filled fields. Then immediately output RESULT:APPLIED with note 'dry run - form filled but not submitted'.\n\nDO NOT proceed past this point. DO NOT click any submission buttons."
    else:
        submit_instruction = "BEFORE clicking Submit/Apply, take a snapshot and review EVERY field on the page. Verify all data matches the APPLICANT PROFILE and TAILORED RESUME -- name, email, phone, location, work auth, resume uploaded, cover letter if applicable. If anything is wrong or missing, fix it FIRST. Only click Submit after confirming everything is correct."
        mission_text = "Submit a complete, accurate application. Use the profile and resume as source data -- adapt to fit each form's format."
...
{"- *** DRY RUN MODE: NEVER click Submit, Apply, Send, or any final submission button. If you see a submit button, STOP immediately and output RESULT:APPLIED with note 'dry run - stopped before submit'. ***" if dry_run else ""}
...
7. COVER LETTER: Actively search for cover letter fields (both required AND optional). Look for:
   - Text areas labeled "cover letter", "cover letter text", "why are you interested", "additional information", "message", "notes", "comments"
   - File upload fields for "cover letter", "cover letter PDF", "additional documents"
   - Any field that asks for a letter, message, or additional information beyond the resume
   If you find ANY cover letter field (required or optional), fill it based on the field type:
   - TEXT FIELD (textarea, input type="text") -> paste the COVER LETTER TEXT from the prompt above
   - FILE UPLOAD FIELD (input type="file", "Upload" button) -> use browser_file_upload with the cover letter PDF path: {cl_upload_path or "N/A"}
   - If both text and file upload options exist -> prefer the file upload (PDF looks more professional)
   IMPORTANT: Even if the field is marked "optional", fill it. A well-written cover letter increases your chances.
